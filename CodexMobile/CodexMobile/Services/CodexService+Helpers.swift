...
extension CodexService {
    ...
    // Uses server thread metadata as a cheap freshness hint so reopen can decide
    // whether the cached local tail is probably stale before hitting the network.
    func threadLikelyHasNewerRemoteTail(_ threadId: String, tolerance: TimeInterval = 1.0) -> Bool {
        guard let serverUpdatedAt = thread(for: threadId)?.updatedAt else {
            return false
        }

        guard let latestLocalMessageAt = messagesByThread[threadId]?
            .map(\.createdAt)
            .max() else {
            return false
        }

        return serverUpdatedAt.timeIntervalSince(latestLocalMessageAt) > tolerance
    }
    ...
    private func loadSingleSubagentThreadMetadataIfNeeded(threadId: String) async -> Bool {
        ...
        do {
            ...
            if var decodedThread = decodeModel(CodexThread.self, from: threadValue) {
                decodedThread.syncState = thread(for: threadId)?.syncState ?? decodedThread.syncState
                upsertThread(decodedThread)
                return true
            }
        } catch {
            // Keep sidebar expansion lightweight. If a runtime rejects camelCase,
            // retry the metadata read with snake_case once before giving up.
            if shouldRetryThreadReadTurnSnapshotWithSnakeCase(error) {
                do {
                    ...
                    debugSyncLog("subagent metadata read failed thread=\(threadId): \(error.localizedDescription)")
                }
            } else {
                debugSyncLog("subagent metadata read failed thread=\(threadId): \(error.localizedDescription)")
            }
