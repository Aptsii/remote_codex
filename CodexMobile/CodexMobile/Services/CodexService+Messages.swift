...
extension CodexService {
    ...
    func prepareThreadForDisplay(threadId: String) async -> Bool {
        ...
        let existingMessages = messages(for: threadId)
        let hasLocalTail = !existingMessages.isEmpty
        let hasActiveOrRunningTurn = threadHasActiveOrRunningTurn(threadId)
        let isSubagentThread = thread(for: threadId)?.isSubagent ?? false
        ...
        // Tail-first reopen: if we already have local messages and the thread is
        // idle, show that cached tail immediately and refresh only the latest
        // server tail. Older history remains explicit via "Load earlier messages".
        if hasLocalTail && !hasActiveOrRunningTurn {
        ...
        // Idle subagent child threads are prone to oversized `thread/resume`
        // payloads because their local session history can be large even though
        // the user only needs the latest tail when opening them from the sidebar.
        // Prefer the lightweight tail snapshot first and only use heavier resume/
        // history paths when the child is actively running.
        if isSubagentThread && !hasActiveOrRunningTurn {
            let didLoadTail = await loadThreadTailSnapshotIfAvailable(threadId: threadId)
            if didLoadTail {
        ...
        do {
            ...
            if isSubagentThread {
                ...
                if !hasLocalTail {
                    do {
                        ...
                        if shouldTreatAsThreadNotFound(historyError) {
                            handleMissingThread(threadId)
                            return false
                        }
                    }
            ...
            // Non-subagent threads can still use a full history fallback when
            // the light resume path fails and there is no local tail to show.
            if !hasLocalTail {
                do {
                    try await loadThreadHistoryIfNeeded(
                        threadId: threadId,
        ...
        if isSubagentThread && messages(for: threadId).isEmpty {
            let didLoadTail = await loadThreadTailSnapshotIfAvailable(threadId: threadId)
            if didLoadTail {
                guard !Task.isCancelled else {
                    return false
        ...
        if hasActiveOrRunningTurn {
            // Rehydrate in-flight turn metadata only for threads that are already
            // believed to be running. Doing this for every reopened subagent child
            // would immediately fall back to a heavyweight `thread/read` call.
    ...
    // Keeps reopen fast while still recovering older structured rows such as
    // subagent actions in the background after the tail is already visible.
    func scheduleBackgroundFullHistoryHydrationIfNeeded(for threadId: String) {
        guard !hasLoadedFullHistory(threadId: threadId),
        ...
        Task {
            do {
                ...
                debugSyncLog("background full history hydration failed for \(threadId): \(error.localizedDescription)")
            }
        }
    }
    ...
    func loadThreadHistoryIfNeeded(
        ...
        let alreadyHydrated = markHydratedWhenNotMaterialized
            ? hydratedThreadIDs.contains(threadId)
            : fullHistoryHydratedThreadIDs.contains(threadId)
        if !forceRefresh, alreadyHydrated {
            return
        }
        ...
        fullHistoryHydratedThreadIDs.insert(threadId)
        hydratedThreadIDs.insert(threadId)
    }

    func hasLoadedFullHistory(threadId: String) -> Bool {
        fullHistoryHydratedThreadIDs.contains(threadId)
    }

    func loadThreadTailSnapshotIfAvailable(threadId: String, limit: Int = 24) async -> Bool {
        guard isConnected, !threadId.isEmpty else {
            return false
        }
        ...
        do {
            let response = try await sendRequest(method: "thread/readTail", params: params)
            guard let resultObject = response.result?.objectValue else {
                return false
            }

            let tailMessages = decodeMessagesFromThreadTail(threadId: threadId, resultObject: resultObject)
            ...
            let runningIDs = runningThreadIDs
            let merged = await Task.detached {
                Self.mergeHistoryMessages(existingMessages, tailMessages, activeThreadIDs: activeThreadIDs, runningThreadIDs: runningIDs)
            }.value
            ...
            debugSyncLog("thread/readTail failed thread=\(threadId): \(error.localizedDescription)")
            return false
        }
    }

    func loadFullThreadHistoryOnDemand(threadId: String) async {
    ...
    func decodeMessagesFromThreadTail(threadId: String, resultObject: [String: JSONValue]) -> [CodexMessage] {
        let tailItems = resultObject["messages"]?.arrayValue ?? resultObject["tailItems"]?.arrayValue ?? []
        var result: [CodexMessage] = []

        for value in tailItems {
            guard let object = value.objectValue,
                  let roleRaw = object["role"]?.stringValue,
                  let role = CodexMessageRole(rawValue: roleRaw) else {
                continue
            }

            let kind = object["kind"]?.stringValue.flatMap(CodexMessageKind.init(rawValue:)) ?? .chat
            let text = object["text"]?.stringValue?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            guard !text.isEmpty else {
                continue
            }

            let createdAt = decodeHistoryTimestamp(from: object) ?? Date()
