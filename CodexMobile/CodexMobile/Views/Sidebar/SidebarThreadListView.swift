...
struct SidebarThreadListView: View {
    ...
    private func revealSelectedSubagentAncestors() {
        ...
        if visibleChildSubagentCount(for: selectedThread) > 0 {
            expandedSubagentParentIDs.insert(selectedThread.id)
        }
    }
    ...
    private func visibleChildSubagentCount(for thread: CodexThread) -> Int {
        guard let containingGroup = groups.first(where: { $0.contains(thread) }) else {
            return 0
        }

        let hierarchy = SidebarSubagentHierarchy(groupThreads: containingGroup.threads)
        return hierarchy.childrenByParentID[thread.id]?.count ?? 0
    }
}
